...
sifrePovratnaNaknadaUpdate.Rows[i]["id"].ToString());
       ...
       + "All Graphics Types|*.pdf;*.jpg";
        ...
        {
            List<Entities.PodaciKalkulacije> podaciKalkulacije = new List<Entities.PodaciKalkulacije>();
            ...
            for (int i = 0; i < dt_mdg0.Rows.Count; i++)
            {
                try
                {
                    ...
                    string sqlUnosKalkulacije = "INSERT INTO kalkulacija(broj,id_partner,racun,otpremnica,racun_datum,otpremnica_datum,mjesto_troska,datum," +
                                                                        "godina,ukupno_vpc,ukupno_mpc,tecaj,id_valuta,fakturni_iznos," +
                                                                        ...
                                                                        (Convert.ToDouble(dt_mdg0.Rows[i]["IZNOS"].ToString()) * 0.8) + "','" + dt_mdg0.Rows[i]["IZNOS"].ToString() +
                    ...
                    podaciKalkulacije.Add(new Entities.PodaciKalkulacije(sljedeci_unos, int.Parse(dt_mdg0.Rows[i]["BROJ"].ToString())));
            ...
            UnesiStavkeKalkulacijeIscitaneIzExcela(dt_mdg0, dt_mdg0_mda, podaciKalkulacije);
        }
        private void UnesiStavkeKalkulacijeIscitaneIzExcela(DataTable dt_mdg0, DataTable dt_mdg0_mda, List<Entities.PodaciKalkulacije> listaBrojevaKalkulacije)
        {
            double prodajniIznosArtikla = 0;
            int i = 0;
            for (int j = 0; j < listaBrojevaKalkulacije.Count; j++)
            {
                for (; i < dt_mdg0_mda.Rows.Count;)
                {
                    try
                    {
                        if (listaBrojevaKalkulacije[j].sifraKalkulacije != int.Parse(dt_mdg0_mda.Rows[i]["BROJ"].ToString()))
                        {
                            break;
                        }
                        double a = Convert.ToDouble(dt_mdg0_mda.Rows[i]["PRO_CIJ"].ToString());
                        prodajniIznosArtikla = a*(1.25);
                        string sqlUnosStavkiKalkulacije = "INSERT INTO kalkulacija_stavke(kolicina,fak_cijena,rabat,prijevoz,carina,marza_postotak,porez,posebni_porez,broj,sifra,vpc,id_skladiste,id_kalkulacija) values('" +
                                                           dt_mdg0_mda.Rows[i]["KOLIKO"].ToString() + "','" +
                                                           dt_mdg0_mda.Rows[i]["PRO_CIJ"].ToString().Replace(",", ".") + "','" +
                                                           dt_mdg0_mda.Rows[i]["RABAT"].ToString() + "','0.00','0,00','0,00','25,00','0,00','" + listaBrojevaKalkulacije[j].brojKalkulacije + "','" + dt_mdg0_mda.Rows[i]["ARTIKL"].ToString() + "','" +
                                                           prodajniIznosArtikla + "','1','" + dt_mdg0_mda.Rows[i]["BROJ"].ToString() + "')";

                        string updateCijeneArtikala = "UPDATE roba SET vpc='" + prodajniIznosArtikla + "', mpc='" +
                                                      prodajniIznosArtikla + "',nc='"+ dt_mdg0_mda.Rows[i]["PRO_CIJ"].ToString()+"' WHERE sifra='"+ dt_mdg0_mda.Rows[i]["ARTIKL"].ToString() + "'";
                        string updateCijeneArtikalaURobaProdaja = "UPDATE roba_prodaja SET vpc='" + prodajniIznosArtikla + "', nc='" +
                                                                  dt_mdg0_mda.Rows[i]["PRO_CIJ"].ToString() + "', kolicina=cast(kolicina as integer)+'" + dt_mdg0_mda.Rows[i]["KOLIKO"].ToString() +
                                                                  "' WHERE sifra='" + dt_mdg0_mda.Rows[i]["ARTIKL"].ToString() + "'";
                        classSQL.insert(sqlUnosStavkiKalkulacije);
                        classSQL.insert(updateCijeneArtikala);
                        classSQL.insert(updateCijeneArtikalaURobaProdaja);
                        i++;
                    }
                    catch (Exception ex)
                    {
