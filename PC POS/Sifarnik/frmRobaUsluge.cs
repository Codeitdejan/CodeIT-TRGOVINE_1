...
ALTER TABLE roba
    OWNER TO postgres;");
        ...
        {
            double praviMPCrobe = 0;
            DataTable provjeraAkoPostoji = new DataTable();
            for (int i = 0; i < dataTableART.Rows.Count; i++)
            {
                string provjera = "SELECT * from roba WHERE sifra='" + dataTableART.Rows[i]["BROJ"].ToString() + "'";
                provjeraAkoPostoji = classSQL.select(provjera, "roba").Tables[0];
                if (provjeraAkoPostoji.Rows.Count == 0)
                {
                    ...
                    //praviMPCrobe= praviMPCrobe = Convert.ToDouble(dataTableART_ARC.Rows[i]["MP_CIJ"].ToString().Replace(",", ".")) + 1.25;
                    string sql = $"INSERT INTO roba(naziv,id_grupa,jm,vpc,mpc,id_zemlja_porijekla,id_zemlja_uvoza,id_partner," +
                             ...
                             dataTableART_ARC.Rows[i]["VP_CIJ"].ToString().Replace(",", ".") + "','" + dataTableART_ARC.Rows[i]["VP_CIJ"].ToString() + "','60','60','1','1','" + dataTableART.Rows[i]["BROJ"].ToString() + "','" + dataTableART.Rows[i]["BAR_KOD"] +
                             "','25,00','DA','0,00','0','0','1','','0','0','','TRUE','FALSE')";
                    classSQL.insert(sql);

                    string sql1 = $"INSERT INTO povratna_naknada(sifra,iznos) values('" + dataTableART.Rows[i]["BROJ"].ToString() + "','0.00')";
                    ...
                    string sql2 = $"INSERT INTO roba_prodaja(id_skladiste,kolicina,nc,vpc,porez,sifra,porez_potrosnja,novo,editirano) values('1','1','" + dataTableART_ARC.Rows[i]["MP_CIJ"].ToString() + "','" +
                                  praviMPCrobe + "','25','" + dataTableART.Rows[i]["BROJ"].ToString() + "','0','TRUE','FALSE')";
                    classSQL.insert(sql2);
                }
            }
